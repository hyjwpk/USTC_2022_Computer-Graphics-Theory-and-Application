...
public:
	...
	void putPixel(int x, int y, const Color& color, int width=1, int height=1) const;
...
bool Tool::init(unsigned width, unsigned height, const char* title)
{
	// GLFW
	//
	// Setup window
	m_width = width * m_point_size;
	m_height = height * m_point_size;
...
bool Tool::windowShouldClose() const
{
	return (bool)glfwWindowShouldClose(m_window);
}

void Tool::pollEvents() const
{
	glfwPollEvents();
}

void Tool::processInput() const
{
	if (glfwGetKey(m_window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
		glfwSetWindowShouldClose(m_window, true);
}

void Tool::putPixel(int x, int y, const Color& color, int width, int height) const
{
...
void Tool::setPixel(int x, int y, const Color& color) const 
{
	int start_x = x * m_point_size;
	int start_y = y * m_point_size;
	for(int i = 0; i < m_point_size; i++) {
		for(int j = 0; j < m_point_size; j++) {
			putPixel(start_x + i, start_y + j, color);
		}
	}
	return;
}

void Tool::flush(float r, float g, float b, float a)
{
...
void Tool::flush(const Color& color)
{
	flush(color.rgba[0], color.rgba[1], color.rgba[2], color.rgba[3]);
}

void Tool::shutdown() const
{
	glfwDestroyWindow(m_window);
	glfwTerminate();
}

GLFWwindow* Tool::getWindow()
{
	return m_window;
}
